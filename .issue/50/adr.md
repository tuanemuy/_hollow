...
- **フォロー (Issue #92)**: 候補B（LIKEフォールバック）を採用。全トークンが3 codepoint未満のときのみ host テーブルを直接 LIKE 検索する経路を searchIndex.ts に追加。SearchKeyword 最小長は据え置き（infrastructure起源の下限をドメインに持ち込まない判断を維持）。トレードオフ: LIKEは全件スキャンで遅い／非索引クエリパスが1つ増える
