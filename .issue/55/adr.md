...
Issue 本文の「背景」は **「進捗表示は、ノート規模が大きいタグ操作（マージ対象ノートが数百件以上）の体験向上を意図したもの」** と明記しており、ユーザーの不安は「規模感が分からない」と「処理中か分からない」の 2 種類。前者は事前提示が、後者が progressbar が解消する。**件数事前提示は単なる UX 改善ではなく Issue 要件の核**として位置づける。
