...
- トレードオフ: モックの `filter-row` と完全一致はしない（＋ chip が無い）。これは意図的なスコープ判断。Issue 冒頭ゴール（レスポンシブ差分を除き完全一致）に対する**意図的な未達分**なので、追跡先として **#654「公開ページ(P30)の『タグを追加(＋)』UI（公開面タグサジェスト）を実装する」を起票**済み（将来の整合性監査で「なぜ＋chipが無いか」が自明になる）。
