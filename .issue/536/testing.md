...
```
file:///Users/hikaru/github.com/tuanemuy/hollow/spec/design/pages/mobile/{画面名}.html
```
...
- **手順:**
  1. アプリ系代表（mobile/P10-home）で サイドバー drawer の開閉（menuBtn → オーバーレイ + backdrop）を確認。
  2. 設定系代表（mobile/P21-settings-profile）で 設定ナビ drawer・セクションカード縦積み・フォーム1カラムを確認。
  3. admin 代表（mobile/P40-admin-dashboard）で nav strip 横スクロール・テーブル→カード変換・メトリクス1カラムを確認。
  4. ダイアログ代表（mobile/P10-move-note-dialog, mobile/common-confirm-dialog, mobile/P13a-upload-modal）で ボトムシート表現（下端立ち上がり・上端角丸・下部フルワイドCTA）を確認。
...
- **手順:** mobile/P11-note-detail / mobile/P33-share-link / mobile/P18-tags を 320px で開き、長文・長URL の折り返しを確認。
- **期待結果:** `overflow-wrap` で折り返し、ページ overflow なし。

### 2. 下端固定要素の競合

- **目的:** bulk-bar と下部固定CTA が同一下端で競合しないか。
- **手順:** mobile/P10-home で選択状態（bulk-bar 表示）にし、下部固定CTAとの重なり・safe-area を確認。
- **期待結果:** CTA がバーに隠れない。`env(safe-area-inset-bottom)` 確保。

## 既存機能への影響確認

- **desktop モックの非改変:** `git diff` で既存 desktop モック `spec/design/pages/*.html`（直下。mobile は `spec/design/pages/mobile/` 配下なので別）に変更が入っていないことを確認（このIssueは新規ファイル追加のみが原則）。
