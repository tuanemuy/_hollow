...
- 失敗時の挙動（review-001 W-001 への対応）:
  - キャッシュには**成功したプロミスのみ**を残す。初回 fetch が reject した場合（一時的な 5xx・ネットワーク断・デプロイ中など）はキャッシュスロットを `undefined` に戻し、次のナビゲーションでリトライ可能にする。`??=` で rejected promise をそのまま保持すると、一度の一過性失敗で以後セッション全体の config 取得が失敗し続け自己回復しなくなる（修正前は per-nav fresh 取得で自然回復していた）ため、この後退を避ける。
