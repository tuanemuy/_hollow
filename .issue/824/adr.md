...
- **AC-5 の caveat 補足（round-1 review Frontend W-001）**: `HeaderCenter` root は全 `/_app` ページ共有の中央 grid アイテムなので、そこに付けた `min-w-0` は編集画面以外でも効き、中央トラックの min サイズ算定を `auto`→`0` に変える。ただし現行の中央要素は検索ボックス（`max-w-[460px]` で有界、通常は中央トラックを押し広げない）なので、`min-w-0` 追加は「万一中身が溢れても縮む」方向の変更で、視覚・挙動は不変（overflow 抑制方向の改善）。manual-test で `/`・`/settings` の非編集ヘッダーが従来どおり検索表示・押し広げなしであることを確認済み。AC-5 は「視覚・挙動が不変」の範囲でこの縮小挙動の差を含む。
