...
### Consequences
- 良い点: commit（source）経路では「行なし blob」という不可視状態が構造的に発生しなくなる。bytes-first のままの `uploadMedia` 経路では metadata 永続化失敗時に従来どおり発生しうる（本 Issue のスコープ外、#452 以来の許容）。回収は既存の purge 機構に一本化され、新しい削除経路が増えない。ポートは不変。
