...
### 補足（レビュー FE-W-003）
Sidebar の `SavedViewsSection` は保存ビュー 0 件時に何も描画しない。「スケルトン → 完全消滅」のレイアウトシフトと SR への空振りアナウンスは残るが、0 件で `null` を返す挙動（サイドバーに空セクション見出しを出さない）は既存仕様として維持し、代わりにフォールバックを最小化（バー 1 本）してシフト量を抑える判断とした。
