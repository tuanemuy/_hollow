...
## ADR-005: 静的な broken-banner に `role="alert"` を使わない（レビュー反映）

### Status
Accepted

### Context
当初 broken-banner（壊れた条件の警告）に `role="alert"` を付けていたが、PR レビュー（review-001）で複数視点から指摘された。`role="alert"` は assertive なライブリージョンで、ロード後の DOM 変化を支援技術に割り込み通知する用途。broken-banner はページロード時点で静的に描画される情報コンテンツであり、(1) ライブリージョンはロード時の静的内容を通知しないため意味がなく、(2) 同一行内の動的な validation/server エラー `<p role="alert">`（こちらは適切）と併存すると複数 alert になる。

### Decision
broken-banner から `role="alert"` を除去する。可視テキスト「壊れた条件があります」＋警告配色＋DOM 順で十分知覚可能。動的に出現する validation/server エラーの `<p role="alert">` は適切なため維持する。

### Consequences
- 良い点: ライブリージョンの誤用を解消し、行内で alert が重複しない。SR 利用者は通常の読み上げ順で警告テキストに到達できる。
- トレードオフ: 壊れた条件の出現をリアルタイム通知はしないが、本ビューは一覧ロード時点の静的表示なので通知の必要がない。
