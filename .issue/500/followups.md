...
> **admin 群（P40〜P46）の B 項目の扱いに関する注記**: 本一覧は原則「モックを正として残置済み（モック側に未実装 UI が残っている）」だが、admin 群の一部項目（P45 の bulk-bar／ユーザー削除／強制ログアウト／ストレージ列、P46 の自動リフレッシュ／タブ／経過・再試行列 など）は例外である。これらは実装が意図的に簡素な構成へ進化した（要判断 重要論点 #8、ADR-004/002 等）ため、対応する A 分類で**モックからは削除済み**であり、モックに残置されているわけではない。ここでは「実装が将来取り込むべき候補（＝モックが描いていた機能で実装に無いもの）」として記録しており、復活させてモック・実装に戻すか否かは `decisions-pending.md` の重要論点 #8（ADR 有効性の確認）の判断に依存する。つまり admin 群のこれら項目は「モック残置」ではなく「実装が正として削除済み・将来実装候補（C #8 依存）」と読むこと。
