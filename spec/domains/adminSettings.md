...
### SpeechProvider（列挙）
- `'openai' | 'deepgram' | 'gemini' | 'deepgram-workers-ai'` — `["openai", "deepgram", "gemini", "deepgram-workers-ai"] as const`。`app/core/adapters/speech/registry.ts` の `speechProviderRegistry`（`Record<SpeechProvider, SpeechAdapter>` でコンパイル時網羅）と対応する。provider ごとの `model` 既定値の対応（`openai → 'gpt-4o-transcribe'`、`deepgram → 'nova-3'`、`gemini → 'gemini-2.5-flash'`、`deepgram-workers-ai → '@cf/deepgram/nova-3'`）も VO 側の INVARIANT として明記する（provider 追加時にモデル既定も増える齟齬を防ぐ）。既定プロバイダは `openai` 据え置き（`defaultSpeech()` 不変）。さらにプロバイダを registry で差し替え可能（Issue #701 ADR-001 / ADR-002・#738 Deepgram・#766 Gemini・#788 Deepgram Workers AI・`spec/adr/013-speech-provider.md`）。
...
- **`deepgram-workers-ai`（Issue #788）は Cloudflare Workers AI ルート**（`env.AI.run("@cf/deepgram/nova-3", ...)`）。認証は Cloudflare 側で解決するため **API キー不要**（keyless）。「この provider は apiKey を要するか」の判定は domain SSOT 述語 `SpeechRecognitionConfig.requiresApiKey(provider)`（keyless 集合 `KEYLESS_SPEECH_PROVIDERS = ["deepgram-workers-ai"]`）に集約し、domain service `assertSpeechEnvOverride`・usecase `updateSpeechConfig`/`testSpeechConnection`・DI `buildSpeechRecognitionProvider`/`resolveConsumerSpeechConfig`・`HttpSpeechConnectionTester` の **6 ゲート**が同一述語で keyless 分岐する。keyless は `apiKeySource: 'env'` / `apiKeyCiphertext: null` 相当で表現し **SecretBox 暗号化は非適用**（保管する鍵が無い）。`env.AI` binding は純データ `SpeechAdapterConfig` に混ぜず `SpeechAdapter.create`/`ping` の `deps.ai` 依存として注入する（ADR-002）。接続テストは binding 存在確認のみ（実 `run` を呼ばない・ADR-005）。**録音 UI 既定の webm/opus 受理は staging で検証中・未確定**（AC-2。dev から実 binding へ到達不可・ADR-006）。REST Deepgram（`nova-3`）と Workers AI Deepgram（`@cf/deepgram/nova-3`）は別プロバイダとして UI に 2 つ並ぶ。詳細は `spec/adr/013-speech-provider.md` の #788 追記節・`.issue/788/adr.md`。
- **OpenAI gpt-4o-transcribe の Workers AI ルートはスコープ外**（#788 ADR-003）: Workers AI パートナーモデルに gpt-4o-transcribe は存在せず（型付きカタログは Whisper 系のみ）、AI Gateway プロキシ経由の別統合が必要。別 Issue へ引き継ぐ。
