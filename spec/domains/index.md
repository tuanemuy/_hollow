...
| `directory.deleted` | `directory.deleted` | Directory.DeleteDirectory | View.HandleDirectoryDeletedEvent（注: Issue #181 で物理 event 化。`Directory.DeleteDirectory` は削除した各ディレクトリ（空ディレクトリ含む）について `directory.deleted` を emit し、`View.HandleDirectoryDeletedEvent` が購読して `directoryId` フィルタの SavedView を broken マークする。これとは別に、配下ノートの `note.trashed` 経由で `view.handleNotePurgedEvent` が `note` 参照の broken marker を立てる経路も併存する。2経路は異なる種類の `BrokenConditionMarker`（`directory` / `note`）を立てる） |
