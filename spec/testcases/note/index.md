...
| 他人のディレクトリへ | MoveNote | `ForbiddenError('NOTE_FORBIDDEN')` |
| 移動先存在せず | MoveNote | `ForbiddenError('DIRECTORY_NOT_FOUND')`（authorization predicate として「アクセス可能なディレクトリのみ存在確認できる」設計） |
...
| 前提条件 | 操作 | 期待結果 |
|---|---|---|
| active ノート | DeleteNote | trashed 化、Outbox `note.trashed`（物理削除ではなく status 遷移のため） |
| 既に trashed | DeleteNote | `BusinessRuleError('note_already_trashed')` |
| trashed ノート | RestoreNote | active、`note.restored` 発火 |
...
| 他人のノート | GetNoteDetail | `ForbiddenError('NOTE_FORBIDDEN')` |
