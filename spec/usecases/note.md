...
4. UoW: `Note.create`、`NoteRepository.save`（タグ件数は読み取り時に集計する。`tags.note_count` 列は Issue #372 で撤去済み。表示件数の真実源は read-time 集計。spec/domains/tag.md 参照）
...
3. UoW:
   ...
   - タグ件数は読み取り時に集計する。`tags.note_count` 列は Issue #372 で撤去済み（表示件数の真実源は read-time 集計。spec/domains/tag.md 参照）
