...
- **タッチターゲット**: 意図は「タッチで押し間違えない」こと。一般のモバイル操作対象は快適なタップサイズ（実装では多くが `max-sm:min-h-[44px]` を維持）を保つ。譲れない床は WCAG 2.5.8 Target Size (Minimum) AA = `24 × 24px`（AA 未満は許さない）。**例外**: グローバルヘッダー（`Header`）の密なコントロール行（検索 input・アップロード/新規作成 CTA・メニューボタン）のみ、検索 input に揃えて 36px（desktop/mobile 共通、h-9 相当）とする（[#628 ADR-003/004](../../.issue/628/adr.md) によるヘッダー局所の例外であり、44px 床の全面撤廃ではない）。形態ごとの適用例は §7.1「タップ領域」、a11y 全体の整合は §8 を参照。
...
- **モバイル専用モック**: 各画面の `spec/design/pages/{name}.html`（デスクトップ基準）に対し、モバイル最適版を `spec/design/pages/mobile/{name}.html` として併置する（同一 basename でデスクトップ↔モバイルが 1:1 対応）。基準幅 390px・320〜430px で overflow=0 を満たし、本節のモバイル方針（単カラム + サイドバー drawer、ダイアログのボトムシート化、admin テーブルのカード化。モバイル操作対象は快適なタップサイズ＝ ≈44px 床を維持し、グローバルヘッダーの操作行のみ局所的に床を外して 36px に揃える。[#628 ADR-004](../../.issue/628/adr.md)）を実体化する。トークンは対応するデスクトップ版の `:root` を逐語コピーし、モバイル専用の新規トークンは定義しない（`--text-*` の `clamp()` が小画面で自動縮小するため。[#536](../../.issue/536/plan.md)）。
...
- **デスクトップ専用で密度優先の文脈では揃えないのが正しい**こともある。管理画面（admin）の小型行アクション（`pillBtn` + `pillBtnSm`。`data-[sm]:h-7`=28px）は、admin がオペレーション画面でモバイル想定が薄く、ジョブテーブル等は行高を抑えた方が一覧性が高いため、密度を優先する（[#292 ADR-005](../../.issue/292/adr.md)。後述「タップ領域」の AA 床は満たす）。
...
意図は「タッチで押し間違えない」こと。一般のモバイル操作対象は快適なタップサイズ（≈44px。実装では多くが `pillBtn` 等の `max-sm:min-h-[44px]` を維持）を保ち、その上で WCAG の譲れない床を守る:

- **譲れない床**: WCAG 2.5.8 Target Size (Minimum) AA = **24×24px**。これを下回る選択はしない。
- **モバイル操作対象は ≈44px 床を維持**: ダイアログ・フォーム・一括操作・カード行アクション・エディタ・管理画面・タグ・公開・認証など、アプリ一般のタッチ操作対象は ≈44px の快適なサイズを保つ（各モバイルモック／実装の `min-height:44px`／`max-sm:min-h-[44px]` は維持。44px 目標の経緯は [#292 ADR-003](../../.issue/292/adr.md) も参照）。
- **例外（ヘッダー局所）**: グローバルヘッダー（`Header`）の密なコントロール行（検索 input・アップロード/新規作成 CTA・メニューボタン）**のみ**、検索 input に揃えて **36px**（desktop/mobile 共通、h-9 相当）に統一する（[#628 ADR-003/004](../../.issue/628/adr.md)）。ここだけヘッダー局所の上書きで 44px 床を外す。これはヘッダー操作行の限定的な例外であり、44px 床の全面撤廃ではない。アイコンのみボタンは同サイズの正方形。

文脈判断の例:

- グローバルヘッダー（検索 / アップロード / 新規作成 / メニュー）は検索 input に揃え desktop/mobile 共通 36px（h-9 相当）。アイコンのみボタンは同サイズの正方形。ヘッダー局所でのみ 44px 床を外す例外。
- admin の `pillBtnSm`（`data-[sm]:h-7`=28px）＝デスクトップ密度優先で 24 床を満たす例。
- `Dialog` の `dialogCloseButton`（`w-8 h-8`=32px）＝32px で 24 床を満たす例。
...
- レスポンシブは `min-width` ベース。例外として、特定のブレークポイント範囲のみに適用したいスタイル（例: モバイルでのヘッダー CTA のアイコン化）では `max-width` メディアクエリも使用してよい。
