...
- **モバイル専用モック**: 各画面の `spec/design/pages/{name}.html`（デスクトップ基準）に対し、モバイル最適版を `spec/design/pages/{name}-mobile.html` として併置する。基準幅 390px・320〜430px で overflow=0 を満たし、本節のモバイル方針（単カラム + サイドバー drawer、ダイアログのボトムシート化、admin テーブルのカード化、タッチ床 44px）を実体化する。トークンは対応するデスクトップ版の `:root` を逐語コピーし、モバイル専用の新規トークンは定義しない（`--text-*` の `clamp()` が小画面で自動縮小するため。[#536](../../.issue/536/plan.md)）。
