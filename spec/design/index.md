...
ボタンには「アイコン+ラベル」「テキストのみ」「アイコンのみ」の 3 形態がある。判断基準と a11y 要件を以下に定める。

- **アイコン+ラベル（既定）**: 主要アクションのデフォルト形態。ヘッダーのナビゲーション、ノート一覧ツールバー、一括操作バー、行アクション、ノート詳細アクション、確認ダイアログの confirm ボタン、空状態 CTA、管理画面セクションヘッダ等で使う。例: 「編集」「公開設定」「ゴミ箱へ」「再実行」「すべてのノートに戻る」等。
- **テキストのみ**: 以下の場面でのみ使う:
  - **タブ／セグメント／トグルグループ**（例: `DisplayModeSwitch` のリスト／タイル／カレンダー切替）— 選択 UI として独立したコントロールカテゴリ
  - **フィルタチップ**（chip 内のテキストはチップ自体が形態であり、別途アイコンを足さない。chip 内付属の close × は後述の通り例外扱い）
  - **リンク的ボタン**（以下 3 条件をすべて満たすもの: ①`<a>` で実装される、②視覚スタイルが下線・テキスト色のみで pill 形状ではない、③周囲にボタン形状の primary CTA が無い。例: パブリックヘッダーの「サインアップ」(`PUBLIC_TEXT_LINK_SIGNUP`)。隣接する「ログイン」は `PILL_BTN` の primary CTA であり、本カテゴリには該当しない — ②③ の判定を行う際の対照例として参照）
- **アイコンのみ**: 以下の要件を満たす必要がある（MUST/SHOULD を区別する）:
  - **MUST**: `aria-label` を `<button>` 自身に必ず付ける（`Icon` の `label` は使わず、accessible name の二重化を避ける）
  - **MUST**: 同種の icon-only ボタンを並置しない（並ぶと意味が判別しづらくなる）
  - **MUST**: タップ領域は **44×44px 以上**（§3 のレスポンシブ戦略と整合）
  - **SHOULD**: `title` 属性も併記してホバー時のヒントを担保する（hover 環境のみ有効。タッチデバイスでは恩恵が無いため、ホバー非依存原則 §4 のもとで推奨レベルに留める）
- **同一ツールバー / アクション群内では形態を揃える**（混在禁止）。ただし以下 2 つの例外を許容する:
  - (a) **タブ／セグメント／トグルグループは選択 UI として混在ルールの対象外**（`DisplayModeSwitch` のような切替系。ADR-002）
  - (b) **密度差を出したい場合は primary をアイコン+ラベル / secondary をテキストのみで揃える**（アイコンのみは混ぜない。`ConfirmDialog` の「ゴミ箱へ」/「キャンセル」の組合せが該当例）
- **空状態 CTA はアイコン+ラベル**（例: ゴミ箱が空のときの「すべてのノートに戻る」+ `ArrowLeft`）。空状態アイキャッチ自体については §7.1 本文の通り、認証済み側 `EMPTY_STATE` を対象とし、公開側 `EMPTY_LIST` は #231 の初期スコープ外。
- **チップ内付属の close ボタンは親 chip のタップ領域と一体扱い**（FilterBar のアクティブチップ解除 "×" は 16px 相当のアイコンを許容する。chip 自体が 28px 高で、× は親 chip の操作領域内に内包される付属要素のため、独立した icon-only ボタンの 44px 要件の対象外）。
- **管理画面（admin）の小型行アクション (`h-7` / `BTN_SM_CLASS`) は情報密度優先のデスクトップ前提**として §3 44×44px 要件の例外とする（ADR-005）。admin はオペレーション画面でモバイル想定が薄く、ジョブテーブル等の行高を抑えた方が一覧性が高い。将来モバイル対応が必要になった場合は再設計する。
