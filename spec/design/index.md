...
- **テキストのみ**: 以下の場面でのみ使う:
  ...
  - **リンク的ボタン**（以下 3 条件をすべて満たすもの: ①`<a>` で実装される、②視覚スタイルが下線・テキスト色のみで pill 形状ではない、③周囲にボタン形状の primary CTA が無い。例: パブリックヘッダーのサインアップ／ログイン）
...
- **空状態 CTA はアイコン+ラベル**（例: ゴミ箱が空のときの「すべてのノートに戻る」+ `ArrowLeft`）。空状態アイキャッチ自体については本節冒頭の通り、認証済み側 `EMPTY_STATE` を対象とし、公開側 `EMPTY_LIST` は §7.1 本文の通り（#231 の初期スコープ外）。
