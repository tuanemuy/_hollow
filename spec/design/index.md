...
- **使う場面**: 主要アクションボタン（ヘッダーのナビゲーション、ノート一覧ツールバー、一括操作バー、行アクション、ノート詳細アクション、確認ダイアログのアイキャッチ、空状態のアイキャッチ、管理画面セクションヘッダ）には、既定としてアイコン+テキストが向く。**空状態のアイキャッチについては** 認証済み側の `EMPTY_STATE`（`layout/styles.ts`）を使う箇所を対象とし、公開側の `EMPTY_LIST`（`public/styles.ts`）は将来の対応とする（#231 の初期スコープ外）。
...
**空状態 CTA** はアイコン+ラベルが向く（例: ゴミ箱が空のときの「すべてのノートに戻る」+ `ArrowLeft`）。空状態アイキャッチ自体については §7.1 本文の通り、認証済み側 `EMPTY_STATE` を対象とし、公開側 `EMPTY_LIST` は #231 の初期スコープ外。
