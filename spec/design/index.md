...
- **テキストのみ**: 以下の場面でのみ使う:
  ...
  - **フィルタチップ**（chip 内のテキストはチップ自体が形態であり、別途アイコンを足さない）
  - **リンク的ボタン**（パブリックヘッダーのサインアップ／ログイン等、テキストリンクとしての位相が強いもの）
- **アイコンのみ**: 以下 4 つの要件を **すべて** 満たす必要がある:
  - `aria-label` を `<button>` 自身に必ず付ける（`Icon` の `label` は使わず、accessible name の二重化を避ける）
  - `title` 属性も併記してホバー時のヒントを担保する
  - 同種の icon-only ボタンを並置しない（並ぶと意味が判別しづらくなる）
  - タップ領域は **44×44px 以上**（§3 のレスポンシブ戦略と整合）
