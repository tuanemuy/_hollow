...
- **ノート詳細は単一カラム**で構成する。本文の上部はタイトルと公開状態など最小限に保ち、メタ情報（作成日 / 更新日 / 場所 / タグ）は本文の**下**に置く。バックリンクは**カード型リスト 1 箇所**に集約し、右メタレールには分散させない。これは読書への集中を最優先し、上部・側方の占有を解消する判断で、実装（#356 ADR-002/003）がこの方向へ移行済みであることに整合させたもの（旧 §2.1 は 3 カラム + 右メタレールを定義していたが、右レールはメタを二重化し読書面を狭めるため廃止した）。
