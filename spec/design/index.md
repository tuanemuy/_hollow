...
- **タッチターゲット**: 意図は「タッチで押し間違えない」こと。WCAG の二段で考える — 譲れない床は 2.5.8 Target Size (Minimum) AA、望ましい目標は 2.5.5 Target Size (Enhanced) AAA（本プロジェクトの既定目標値）。タッチ主体の文脈（モバイル / `pointer: coarse`）では目標を確保する（base ピルはモバイルでタッチ床を担保し、デスクトップは自由）。デスクトップ専用でマウス精度が前提の密度優先 UI（admin 等）は AA を床に密度を取る判断を許す（AA 未満は許さない）。タッチ床はモバイルで共通化し、実装側の一箇所で管理する。形態ごとの適用例は §7.1「タップ領域」、a11y 全体の整合は §8 を参照。
...
- **デスクトップ専用で密度優先の文脈では揃えないのが正しい**こともある。管理画面（admin）の小型行アクションは、モバイルのタッチ床を明示的に opt-in で解除して行高を抑える。admin がオペレーション画面でモバイル想定が薄く、ジョブテーブル等は行高を抑えた方が一覧性が高いため、密度を優先する（[#292 ADR-005](../../.issue/292/adr.md)。後述「タップ領域」の AA 床は満たす）。
...
- base `pillBtn` のモバイルタッチ床 ＝「タッチでは目標床を担保し、デスクトップは自由」という新原則そのもの。
- admin の小型行アクション（base のタッチ床を opt-in で解除）＝デスクトップ密度優先で AA 床を満たす例。
...
- レスポンシブは `min-width` ベース。例外として、グローバルな mobile fix（タッチ床の確保等）や、特定のブレークポイント範囲のみに適用したいスタイルでは `max-width` メディアクエリも使用してよい。
