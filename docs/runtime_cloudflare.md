...
| ----------- | ------------------------------------------------------------------------------------------------------ | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| App (fetch) | TanStack Start HTTP request handling                                                                   | _(top level)_    | `DB` (D1), `TEMP_FILES` / `OBJECT_STORAGE` (R2), `RELAY` (Service Binding), `ASSETS` — dispatch-side secrets `SECRET_BOX_MASTER_KEY`, `ADMIN_LLM_API_KEY`, `ADMIN_SPEECH_API_KEY`, `R2_*`; web-only `ADMIN_SETUP_TOKEN` | HTTP                                                 |
...
`pnpm dev` (Vite + `@cloudflare/vite-plugin`) and `pnpm start` (`wrangler dev` running the Vite build output) boot only the main app Worker. The sibling `relay` / `consumer` / `pruner` / `dlq` Workers and Cloudflare Queues are **not** running, so the default Service Binding → Queue → consumer chain has no producer or consumer attached. To keep UoW-emitted events visible in local dev, the **dev entry** (`app/server.cloudflare.dev.ts`) injects `InlineRelayTrigger` (`app/core/adapters/cloudflare/inlineRelayTrigger.ts`) via `RequestServerConfig.relayTriggerOverride` behind a runtime gate (Issue #66 / #663 / #675):

- **Entry separation (build-time):** `vite.config.cloudflare.ts` selects `server.cloudflare.dev.ts` in every non-production mode and the dev-import-free `server.cloudflare.ts` in `production`. `InlineRelayTrigger` is imported only from the dev entry, so production bundles have no import path to it — the guarantee is structural (no reachable module), not dead-code elimination. Never add `DEV_INLINE_RELAY` to `wrangler.staging.toml` / `wrangler.production.toml` — the adapter is unreachable in production regardless, but keep the intent unambiguous.
- **Runtime gate:** `resolveInlineRelayGate({ viteDev, flag })` (evaluated only inside the dev entry) enables the path when `import.meta.env.DEV === true` (`pnpm dev`) **or** `DEV_INLINE_RELAY === "true"` (local `wrangler.toml [vars]` — this is what carries `pnpm start`). Under `pnpm dev` the Vite plugin also supplies the local `[vars]`, so both OR conditions are true; harmless, `viteDev` alone suffices.

To enable the inline path under `pnpm start`, build with **`pnpm build:local && pnpm start`**, not plain `pnpm build`. `pnpm build` (`@cloudflare/vite-plugin`) writes `.wrangler/deploy/config.json`, which redirects `wrangler dev` to `dist/server/wrangler.json` → `dist/server/index.js` — i.e. `pnpm start` always runs the Vite build output, never the TS sources. `pnpm build` runs in `production` mode, so it selects the prod entry (no inline path exists in the bundle); `pnpm build:local` (`vite build --mode development` with `NODE_ENV=production`) selects the dev entry, which keeps the inline path. In that dev-entry output `import.meta.env.DEV` is `false` (because `NODE_ENV=production`), so `DEV_INLINE_RELAY === "true"` alone drives the inline relay — do not collapse `resolveInlineRelayGate` to `viteDev`, or `pnpm start` regresses to Issue #663 ("待機中"). **Beware: under plain `pnpm build && pnpm start` the var is silently inert** — the prod entry was built, so the path does not exist in the bundle and there is no runtime warning. Note also that the redirected config bakes `wrangler.toml [vars]` into `dist/server/wrangler.json` at build time, so after changing any `[vars]` value (e.g. `DEV_INLINE_RELAY`) you must re-run `pnpm build:local` — restarting `pnpm start` alone does not pick the change up.
...
- Production / staging deploys (`pnpm build` → `wrangler deploy`) take the **unchanged** Service Binding path. `pnpm build` (production mode) selects the prod entry, which never imports `InlineRelayTrigger`, so the adapter has no import path into the deployed bundle — it is structurally absent, not dead-code-eliminated.
...
ローカル検証（`pnpm build:local && pnpm start` = `wrangler dev`, `http://localhost:8787`）で presigned アップロード/ダウンロードフロー（presign → ブラウザ PUT → finalize → 表示）を E2E 完走させる仕組み（Issue #657 / #675）。プロキシ（`buildDevObjectStorageResponse`）は dev エントリ（`app/server.cloudflare.dev.ts`）からのみ配線される。`pnpm build:local` は dev エントリを選ぶのでプロキシが載るが、素の `pnpm build`（production mode）は prod エントリを選ぶため import 経路が無く、プロキシは production バンドルに構造的に載らない。E2E 完走には `pnpm build:local` が必須（[Local dev outbox dispatch](#local-dev-outbox-dispatch) 参照）。
...
- `R2_DEV_OBJECT_PROXY = "true"` のときだけ、dev エントリ（`app/server.cloudflare.dev.ts`）の `preRoute` フックが `/dev/r2/<bucket>/<key>` への PUT/GET を `buildDevObjectStorageResponse`（`app/core/adapters/cloudflare/devObjectStorageHandler.ts`）に委譲する。終端はローカル miniflare の `OBJECT_STORAGE` binding なので、finalize の `stat` と同一ストアになる。
...
- `R2_OBJECT_BUCKET_NAME` / `ADMIN_LLM_MODEL` / `ADMIN_SPEECH_MODEL` / `ADMIN_SPEECH_PROVIDER` (vars) — public configuration that complements the secrets listed below.
...
| `ADMIN_SPEECH_API_KEY`    | Env override for the OpenAI speech (transcription) api key. **Speech adapter wire condition**: DI wires the real OpenAI speech provider only when this **and** `ADMIN_SPEECH_MODEL` (the `[vars]` entry, public) are **both** present. Either one missing → DI keeps `StubSpeechRecognitionProvider`. When unset, the `/admin/speech` DB save value (SecretBox-encrypted) is used instead (env > db > stub). There is no `ADMIN_SPEECH_BASE_URL` — the speech config carries no base URL override. |
...
- Leaving `ADMIN_SPEECH_API_KEY` empty (or omitting `ADMIN_SPEECH_MODEL` in `wrangler.toml [vars]`) → DI keeps `StubSpeechRecognitionProvider`, so audio ingestion fails as before with `BusinessRuleError("unsupported_format")`. Set the `/admin/speech` DB value (SecretBox-encrypted) or both env vars to wire the real OpenAI speech adapter.
...
The login server-function POST path (`loginFn`, `createServerFn({ method: "POST" })`) has **no application-layer rate limit or lockout**, and by design it never gets one: request-frequency control is deferred to the Cloudflare edge, consistent with the §Observability policy above (`.issue/647/adr.md` ADR-006) and decided for the auth path in `.issue/457/adr.md` (ADR-001 / ADR-002). The edge runs **before** the Worker boots, so it sheds volumetric floods without booting the Worker or spending a D1 write for a counter — costs an in-app limiter cannot avoid. (An in-app limiter checking before `verifyPassword` could still skip the scrypt CPU (`N=2^16`, ~50–100 ms per verify); the edge's structural edge is that it never pays the Worker invocation or the counter's D1 write at all — see `.issue/457/investigation.md` §6.)

Configure a **WAF Rate Limiting Rule** per zone:

- **Match:** the login server-function POST path — method `POST` on the login endpoint. Confirm the concrete path against the deployed server-function route rather than hardcoding a guess. Note that TanStack Start server functions are posted to a build-derived `_serverFn` path (function ID), which can change across deploys when files are moved or renamed — a rule pinned to a specific path can **silently stop matching** with no error. Re-verify the rule after each deploy, or match a coarser but stable attribute (e.g. `POST` on the `_serverFn` base path) as a backstop, accepting that a coarse match also rate-limits other mutations sharing that path.
