...
- **Repeated utility strings** can be hoisted into a module-scoped string constant (see `app/components/common/styles.ts`, `auth/styles.ts`, `layout/styles.ts`, `public/styles.ts`). Tailwind's JIT scans those literals, so behavior is identical to inline.
