      ...
      {notes.length === 0 ? (
        ...
        <ul className="mt-2 list-none p-0 m-0">
          {notes.map((note) => (
            <li
              key={note.id}
              className="grid grid-cols-[1fr_auto] gap-4 px-3 py-5 border-t border-hairline transition-colors motion-reduce:transition-none items-start hover:bg-surface max-sm:grid-cols-1 max-sm:gap-3 max-sm:px-2"
            >
