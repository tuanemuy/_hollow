...
// to keep accepting both formats after the scrypt swap so existing
...
describe("ScryptPasswordHasher", () => {
  it("hash() produces a scrypt PHC-encoded string", async () => {
  ...
  it("verify() returns false (does not throw) for malformed input", async () => {
    ...
    await expect(hasher.verify("x", "$scrypt$totally-broken")).resolves.toBe(
      false,
    );
    await expect(
