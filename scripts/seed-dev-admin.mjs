...
// Idempotent. The user row is upserted, never deleted: deleting it cascades
// into the dev admin's notes / directories / media, whose RESTRICT
// back-references (notes.directory_id → directories, directories.parent_id →
// directories) abort the cascade with SQLITE_CONSTRAINT_TRIGGER once any
// nested data exists — and even when the cascade succeeds it destroys data
// we want to keep. Re-asserting the canonical admin state via ON CONFLICT
// leaves owned data intact (Issue #512). Sessions are leaf rows (nothing FK-
// references them), so a delete-then-insert is safe and also clears the
// fixed token if it was attached to a different user.
const SQL = `
